...
void printDeclType(raw_ostream &out, PrintingPolicy const &policy, QualType T,
...
void printTemplateParameters(raw_ostream &out, PrintingPolicy const &policy,
...
// Adapted from tools\clang\lib\AST\DeclPrinter.cpp
// DeclPrinter::PrintTemplateParameters
void printTemplateParameter(raw_ostream &out, PrintingPolicy const &policy,
                            NamedDecl const *param) {
...
void printTemplateName(raw_ostream &OS, const PrintingPolicy &policy,
                       TemplateName const &name, bool qualifyNames = false) {
  if (auto Template = name.getAsTemplateDecl())
    OS << (qualifyNames ? Template->getQualifiedNameAsString()
                        : Template->getNameAsString());
  else if (auto QTN = name.getAsQualifiedTemplateName()) {
    OS << (qualifyNames ? QTN->getDecl()->getQualifiedNameAsString()
                        : QTN->getDecl()->getNameAsString());
...
void printTemplateArgument(raw_ostream &out, const PrintingPolicy &policy,
                           TemplateArgument const &arg, bool qualifyNames) {
  switch (arg.getKind()) {
...
void printTemplateArguments(raw_ostream &out, const PrintingPolicy &policy,
                            TemplateArgumentList const *args,
                            bool qualifyNames) {
  if (args == nullptr) {
    return;
  }
...
std::string getFunctionPrototype(FunctionDecl *f, bool qualifyNames) {
  ...
  os << getTypeName(f->getReturnType(), qualifyNames) << ' '
     << f->getNameAsString();
  if (f->getTemplatedKind() ==
          FunctionDecl::TK_FunctionTemplateSpecialization ||
      //            f->getTemplatedKind() ==
      //            FunctionDecl::TK_MemberSpecialization ||
      f->getTemplatedKind() ==
          FunctionDecl::TK_DependentFunctionTemplateSpecialization) {
    printTemplateArguments(os, policy, f->getTemplateSpecializationArgs(),
                           qualifyNames);
  }
  if (f->getTemplatedKind() == FunctionDecl::TK_FunctionTemplate) {
    printTemplateParameters(
        os, policy, f->getDescribedFunctionTemplate()->getTemplateParameters());
  }
  os << '(';
  bool first = true;
  for (auto param : f->parameters()) {
    ...
    os << getTypeName(param->getType(), qualifyNames) << ' '

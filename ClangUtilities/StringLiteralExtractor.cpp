...
#pragma warning(push)
#pragma warning(                                                               \
    disable : 4100 4127 4800 4512 4245 4291 4510 4610 4324 4267 4244 4996 4146)
...
// This function is an direct adaptation from ProcessCharEscape in
// llvm-3.7.1\src\tools\clang\lib\Lex\LiteralSupport.cpp (just removed the diag
// part)
unsigned ProcessCharEscape(const char *ThisTokBegin, const char *&ThisTokBuf,
...
// This function is an direct adaptation from ProcessUCNEscape in
// llvm-3.7.1\src\tools\clang\lib\Lex\LiteralSupport.cpp (just removed the diag
// part)
...
// This function is just the same as convertUTF16ToUTF8String, but adapted to
// UTF32, since it did not exist in clang
bool convertUTF32ToUTF8String(ArrayRef<char> SrcBytes, std::string &Out) {
  ...
  const llvm::UTF32 *Src =
      reinterpret_cast<const llvm::UTF32 *>(SrcBytes.begin());
  const llvm::UTF32 *SrcEnd =
      reinterpret_cast<const llvm::UTF32 *>(SrcBytes.end());
...
// This function is an adaptation from StringLiteral::getLocationOfByte in
// llvm-3.7.1\src\tools\clang\lib\AST\Expr.cpp
std::vector<std::string> splitStringLiteral(StringLiteral *S,
  ...
  for (TokNo = 0; TokNo < S->getNumConcatenated(); ++TokNo) {
    ...
    if (TheTok.isAnyIdentifier()) {
      // It should not be, since we are parsing inside a string literal, but it
      // can happen with special macros such as __func__ of __PRETTY_FUNCTION__
      // that are not resolved at this time. In that case, we just ignore
      // them...
      continue;
    }
    ...
    unsigned TokLen =
        Lexer::getSpelling(TheTok, SpellingPtr, SM, Features, &StringInvalid);
    if (StringInvalid)
      continue;

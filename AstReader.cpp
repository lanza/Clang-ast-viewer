...
GenericAstNode::GenericAstNode() : myParent(nullptr), hasDetails(false) {}

int GenericAstNode::findChildIndex(GenericAstNode *node) {
  auto it = std::find_if(myChidren.begin(), myChidren.end(),
                         [node](std::unique_ptr<GenericAstNode> const &n) {
                           return n.get() == node;
                         });
  return it == myChidren.end() ? -1 : it - myChidren.begin();
}
...
struct SourceRangeVisitor : boost::static_visitor<SourceRange> {
  template <class T> SourceRange operator()(T const *t) const {
    if (t == nullptr)
      return SourceRange();
...
bool GenericAstNode::getRangeInMainFile(std::pair<int, int> &result,
  ...
  auto end = manager.getDecomposedSpellingLoc(clang::Lexer::getLocForEndOfToken(
      range.getEnd(), 0, manager, context.getLangOpts()));
...
struct NodeColorVisitor : boost::static_visitor<int> {
  int operator()(Decl const *) const { return 0; }
  int operator()(Stmt const *) const { return 1; }
};
...
void GenericAstNode::setProperty(std::string const &propertyName,
...
public:
  ...
  AstDumpVisitor(clang::ASTContext &context, GenericAstNode *rootNode)
      : myRootNode(rootNode), myAstContext(context) {
  ...
  std::string getMangling(clang::NamedDecl const *ND) {
    if (auto funcContext = dyn_cast<FunctionDecl>(ND->getDeclContext())) {
      if (funcContext->getTemplatedKind() ==
    ...
    for (auto tag : containers) {
      if (auto partialSpe =
              dyn_cast<ClassTemplatePartialSpecializationDecl>(tag)) {
        return "<Inside partial specialization " + tag->getNameAsString() +
               ": " + ND->getNameAsString() + ">";
      } else if (auto recContext = dyn_cast<CXXRecordDecl>(tag)) {
        if (recContext->getDescribedClassTemplate() != nullptr) {
          return "<Inside a template" + tag->getNameAsString() + ": " +
                 ND->getNameAsString() + ">";
        }
      }
    }

    auto mangleContext = std::unique_ptr<clang::MangleContext>{
        ND->getASTContext().createMangleContext()};
    std::string FrontendBuf;
    llvm::raw_string_ostream FrontendBufOS(FrontendBuf);

    if (auto ctor = dyn_cast<CXXConstructorDecl>(ND)) {
      // mangleContext->mangleCXXCtor(ctor, CXXCtorType::Ctor_Complete,
      //                              FrontendBufOS);
    } else if (auto dtor = dyn_cast<CXXDestructorDecl>(ND)) {
      // mangleContext->mangleCXXDtor(dtor, CXXDtorType::Dtor_Complete,
      //                              FrontendBufOS);
    } else if (mangleContext->shouldMangleDeclName(ND) &&
               !isa<ParmVarDecl>(ND)) {
  ...
  bool TraverseDecl(clang::Decl *decl) {
    ...
    node->myAstNode = decl;
    node->name = decl->getDeclKindName() +
                 std::string("Decl"); // Try to mimick clang default dump
    if (auto *FD = dyn_cast<FunctionDecl>(decl)) {
      ...
      bool invalid;
      auto startingLine =
          mngr.getExpansionLineNumber(FD->getLocation(), &invalid);
      ...
      node->setProperty(props::Name,
                        clang_utilities::getFunctionPrototype(FD, true));
      if (auto *MD = dyn_cast<CXXMethodDecl>(FD)) {
        node->setProperty(props::IsGenerated,
                          MD->isUserProvided() ? "False" : "True");
      }
      ...
      if (auto *PFD = dyn_cast_or_null<FunctionDecl>(
      ...
      node->setProperty(props::Type,
                        clang_utilities::getTypeName(VD->getType(), true));
      ...
      if (auto templateInstance =
              dyn_cast<ClassTemplateSpecializationDecl>(tag)) {
        clang::PrintingPolicy policy(
            templateInstance->getASTContext().getLangOpts());
        clang_utilities::printTemplateArguments(
            os, policy, &templateInstance->getTemplateArgs(), false);
      }
  ...
  bool VisitStringLiteral(clang::StringLiteral *s) {
    ...
    myStack.back()->setProperty(props::InterpretedValue, s->getBytes().str());
    auto parts = clang_utilities::splitStringLiteral(
        s, myAstContext.getSourceManager(), myAstContext.getLangOpts(),
        myAstContext.getTargetInfo());
    if (parts.size() == 1) {
      ...
      for (auto &part : parts) {
        ++i;
        myStack.back()->setProperty(props::Value + " " + std::to_string(i),
                                    part);
      }
    }
    return true;
  ...
  bool VisitIntegerLiteral(clang::IntegerLiteral *i) {
    ...
    myStack.back()->setProperty(props::Value,
                                i->getValue().toString(10, isSigned));
    return true;
  }
  ...
  bool VisitFloatingLiteral(clang::FloatingLiteral *f) {
    myStack.back()->setProperty(
        props::Value, std::to_string(f->getValueAsApproximateDouble()));
    return true;
  }

  bool VisitCXXRecordDecl(clang::CXXRecordDecl *r) {
    myStack.back()->setProperty(
        props::IsTemplateDecl,
        std::to_string(r->getDescribedClassTemplate() != nullptr));
    return true;
  }

  void addReference(GenericAstNode *node, clang::NamedDecl *referenced,
                    std::string const &label) {
    auto funcDecl = dyn_cast<FunctionDecl>(referenced);
    myStack.back()->setProperty(
        label, funcDecl == nullptr
...
clang::ASTContext &AstReader::getContext() { return myAst->getASTContext(); }

GenericAstNode *AstReader::getRealRoot() {
  return myArtificialRoot->myChidren.front().get();
}

GenericAstNode *AstReader::findPosInChildren(
...
std::vector<GenericAstNode *>
AstReader::getBestNodeMatchingPosition(int position) {
...
GenericAstNode *AstReader::readAst(std::string const &sourceCode,
  ...
  if (myAst != nullptr) {
    for (auto it = myAst->top_level_begin(); it != myAst->top_level_end();
         ++it) {
      //(*it)->dumpColor();
    }
...
bool AstReader::ready() { return isReady; }

void AstReader::dirty() { isReady = false; }

...
std::vector<std::string> splitCommandLine(std::string const &cmdline) {
...
#ifndef _WIN32
  {
    ...
    if (!(argv = static_cast<char **>(calloc(argc, sizeof(char *))))) {
      goto fail;
    }
...
#else  // WIN32
  {
    ...
    for (i = 0; i < argc; i++) {
      ...
      needed = WideCharToMultiByte(CP_ACP, 0, wargs[i], -1, nullptr, 0, nullptr,
                                   nullptr);
      char *argv;
      ...
      // Do the conversion.
      needed = WideCharToMultiByte(CP_ACP, 0, wargs[i], -1, argv, needed,
                                   nullptr, nullptr);

...
AstModel::AstModel(GenericAstNode *data, QObject *parent)
    : QAbstractItemModel(parent), rootItem(data) {}

AstModel::~AstModel() {}

int AstModel::columnCount(const QModelIndex &parent) const { return 1; }
...
QModelIndex AstModel::rootIndex() const { return createIndex(0, 0, rootItem); }

QModelIndex AstModel::parent(const QModelIndex &index) const {
  if (!index.isValid())
    return QModelIndex();

  GenericAstNode *childItem =
      static_cast<GenericAstNode *>(index.internalPointer());
  ...
  auto parentRow =
      grandFather == nullptr ? 0 : grandFather->findChildIndex(parentItem);

...
class UpdateLock {
public:
  UpdateLock(bool &lock) : myLock(lock) { myLock = true; }
  ~UpdateLock() { myLock = false; }
  bool &myLock;
};

MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent), isUpdateInProgress(false) {
  myUi.setupUi(this);

  myReader = new AstReader;

  connect(myUi.actionRefresh, &QAction::triggered, this,
          &MainWindow::RefreshAst);

  myHighlighter = new Highlighter(myUi.codeViewer->document());
  myUi.nodeProperties->setHeaderLabels({"Property", "Value"});
  connect(myUi.codeViewer, &QTextEdit::cursorPositionChanged, this,
          &MainWindow::HighlightNodeMatchingCode);
  connect(myUi.codeViewer, &QTextEdit::textChanged, this,
          &MainWindow::OnCodeChange);
  connect(myUi.showDetails, &QPushButton::clicked, this,
          &MainWindow::ShowNodeDetails);
}

void MainWindow::RefreshAst() {
  auto ast = myReader->readAst(
      myUi.codeViewer->document()->toPlainText().toStdString(),
  ...
  connect(myUi.astTreeView->selectionModel(),
          &QItemSelectionModel::currentChanged, this,
          &MainWindow::HighlightCodeMatchingNode);
  connect(myUi.astTreeView->selectionModel(),
          &QItemSelectionModel::currentChanged, this,
          &MainWindow::DisplayNodeProperties);
  myUi.astTreeView->setEnabled(myReader->ready());
}

void MainWindow::HighlightCodeMatchingNode(const QModelIndex &newNode,
                                           const QModelIndex &previousNode) {
  if (isUpdateInProgress) {
    return;
  }
  auto lock = UpdateLock{isUpdateInProgress};
  auto node = myUi.astTreeView->model()
                  ->data(newNode, Qt::NodeRole)
...
void MainWindow::DisplayNodeProperties(const QModelIndex &newNode,
                                       const QModelIndex &previousNode) {
  ...
  for (auto &prop : node->getProperties()) {
    new QTreeWidgetItem(myUi.nodeProperties,
                        QStringList{QString::fromStdString(prop.first),
...
void MainWindow::HighlightNodeMatchingCode() {
  ...
  if (!nodePath.empty()) {
    ...
    DisplayNodeProperties(
        currentIndex, currentIndex); // Since we won't use the previous node,
                                     // it's not an issue if it is wrong...
  }
}
...
void MainWindow::ShowNodeDetails() {
  ...
  auto model = myUi.astTreeView->model();
  auto node = myUi.astTreeView->model()
                  ->data(selectionModel->currentIndex(), Qt::NodeRole)
                  .value<GenericAstNode *>();
  if (!node || !node->hasDetails) {
    QMessageBox::warning(this, windowTitle() + " - Error in details",
                         "The currently selected node does not have details",
                         QMessageBox::Ok);
    return;
  }
  ...
  win->setWindowTitle(windowTitle() + " - " +
                      ...
                      QString::fromStdString(node->detailsTitle));
